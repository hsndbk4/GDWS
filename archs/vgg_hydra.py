...
def make_layers(cfg, conv_layer, batch_norm=True):
    layers = []
    in_channels = 3
    if conv_layer == Conv2dStat:
        bn_layer = BatchNorm2dStat
    else:
    ...
    for v in cfg:
        if v == "M":
            ...
            if batch_norm:
                layers += [conv2d, bn_layer(v), nn.ReLU(inplace=True)]
            else:
...
def vgg16(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_uniform', **kwargs):
...
def vgg16_bn(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_uniform', **kwargs):
    n = [i for i in cfgs["16"] if isinstance(i, int)][-1]
    model = VGG_HYDRA(
