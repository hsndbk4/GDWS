...
class BasicBlock(nn.Module):
    def __init__(self, conv_layer, in_planes, out_planes, stride, dropRate=0.0):
        ...
        BatchNorm2d = nn.BatchNorm2d if conv_layer == nn.Conv2d else BatchNorm2dStat
...
class WideResNet(nn.Module):
    def __init__(
        ...
        conv_layer = Conv2dStat if use_stat_layers else nn.Conv2d
        linear_layer = LinearStat if use_stat_layers else nn.Linear
...
def wrn_28_10(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_normal', **kwargs):
...
def wrn_28_4(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_normal', **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for WRN"
    return WideResNet(conv_layer, linear_layer, depth=28, widen_factor=4, **kwargs)


def wrn_28_1(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_normal', **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for WRN"
    return WideResNet(conv_layer, linear_layer, depth=28, widen_factor=1, **kwargs)


def wrn_34_10(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_normal', **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for WRN"
    return WideResNet(conv_layer, linear_layer, depth=34, widen_factor=10, **kwargs)


def wrn_40_2(conv_layer=nn.Conv2d, linear_layer=nn.Linear, init_type='kaiming_normal', **kwargs):
    assert init_type == "kaiming_normal", "only supporting default init for WRN"
